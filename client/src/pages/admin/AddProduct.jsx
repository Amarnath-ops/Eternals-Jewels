...
const AddProduct = () => {
        ...
        if (croppedFile) {
            ...
            if (activeCropIndex === null) {
                ...
                setGalleryPreviews(prev => {
                    const newPrev = [...prev];
                    newPrev[activeCropIndex] = previewURL;
                    return newPrev;
                });
            }
            setSrc(null);
    ...
    const handleGalleryFiles = (files) => {
        ...
        if (currentFiles.length + files.length > 4) {
             setError("productImages", { message: "Maximum 4 additional images allowed." });
             return;
        }

        const validFiles = Array.from(files).filter(file => file.type.startsWith("image/"));
        if (validFiles.length !== files.length) {
             setError("productImages", { message: "Only image files are allowed." });
        }
        ...
        setValue("productImages", updatedFiles, { shouldValidate: true });
        ...
        <div className="flex-1 bg-white min-h-screen p-4 md:p-8 font-sans">
            <div className="max-w-6xl mx-auto w-full">
                ...
                <FormWrapper onSubmit={handleSubmit(onSubmit)} className="w-full space-y-8">
                    ...
                    <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
                        <h2 className="text-xl font-bold mb-6 text-gray-800">Add Product Image</h2>
                        ...
                        <div className="mb-8">
                            ...
                            {errors.thumbnail && <p className="text-red-500 text-xs mt-2">{errors.thumbnail.message}</p>}
                        </div>
                                ...
                                {galleryPreviews.map((url, idx) => (
                                    <div key={idx} className="relative aspect-square rounded-xl overflow-hidden border border-gray-200 group">
                                         ...
                                         <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition flex items-center justify-center gap-2">
                                             <button 
                                                type="button" 
                                                onClick={() => handleCropGalleryImage(idx)}
