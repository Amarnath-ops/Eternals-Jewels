...
const CheckoutPage = () => {
    ...
    const deliveryCharge = 0; // Free as per image
    ...
    const handlePlaceOrder = () => {
        ...
        toast.success("Order functionality to be implemented!");
        console.log({
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12" >
                    ...
                    <div className="bg-[#F8F5F2] p-6 md:p-10 rounded-xl h-fit">
                        ...
                        <div className="space-y-6">
                            {items.map((item) => (
                                <div key={`${item.productId} - ${item.variantId}`} className="flex bg-white p-4 rounded-lg shadow-sm">
                                    ...
                                    <div className="ml-4 grow flex flex-col justify-center">
                                        <h3 className="font-semibold text-gray-900 uppercase tracking-wide text-sm">{item.name}</h3>
                                        <p className="text-xs text-gray-500 mt-1">Quantity : {item.quantity}</p>
                                        <p className="text-lg font-medium text-gray-700 mt-1">₹{item.salePrice.toFixed(2)}</p>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div>
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-lg font-bold uppercase tracking-wide text-gray-900">CHOOSE DELIVERY POINT</h2>
                        </div>
                            ...
                            {addressData?.map((addr) => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-2">
                                        {addr.isDefault && (
                                            <span className="text-[10px] uppercase font-bold text-gray-500 tracking-wider">Default</span>
                                        )}
                                        <button 
                        ...
                        <div className="bg-[#F4F1EE] p-8 rounded-xl">
                            ...
                            <div className="space-y-4">
                                <label className="flex items-center gap-4 cursor-pointer">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${paymentMethod === 'razorPay' ? 'border-black' : 'border-gray-400'}`}>
                                ...
                                <label className="flex items-center gap-4 cursor-pointer">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${paymentMethod === 'cod' ? 'border-black' : 'border-gray-400'}`}>
                                ...
                                <label className="flex items-center gap-4 cursor-pointer">
                                    <div className={`w-5 h-5 rounded-full border flex items-center justify-center ${paymentMethod === 'wallet' ? 'border-black' : 'border-gray-400'}`}>
                            ...
                            <button 
                                ...
                                className="w-full bg-[#8B6D51] text-white font-medium py-4 mt-8 rounded-md uppercase tracking-wide hover:bg-[#725841] transition-colors shadow-md"
                            >
                                Place Order
                            </button>
                        </div>
