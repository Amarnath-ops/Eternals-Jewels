...
const Shop = () => {
  ...
  const { data: productsData, isLoading: isProductsLoading } = useGetProducts({
      ...
      sort: sort === 'priceHigh' ? '-variants.0.salePrice' : sort === 'priceLow' ? 'variants.0.salePrice' : sort === 'a-z' ? 'productName' : sort === 'z-a' ? '-productName' : '-createdAt',
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-sm">
                            <div className="flex items-center justify-between mb-6">
                                <h3 className="font-bold text-lg">Filter By</h3>
                                 {(selectedCategories.length > 0 || selectedMaterials.length > 0 || minPrice || maxPrice || searchTerm) && (
                            ...
                            <div className="mb-8">
                                <h4 className="font-bold text-sm mb-4 uppercase tracking-wider text-gray-800">Categories</h4>
                                <div className="space-y-3">
                                    {categoriesData?.data?.map((cat) => (
                                        <label key={cat._id} className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-4 h-4 border flex items-center justify-center transition-colors ${selectedCategories.includes(cat._id) ? 'bg-black border-black' : 'border-gray-300 group-hover:border-gray-400'}`}>
                                                {selectedCategories.includes(cat._id) && <div className="w-2 h-2 bg-white" />}
                                            </div>
                                            <input 
                                                type="checkbox" 
                                                className="hidden"
                                            ...
                                            />
                                            <span className={`text-sm ${selectedCategories.includes(cat._id) ? 'text-gray-900 font-medium' : 'text-gray-500 group-hover:text-gray-700'}`}>
                                                {cat.categoryName}
                                            </span>
                                        </label>
                                    ))}
                             ...
                             <div className="mb-8">
                                <h4 className="font-bold text-sm mb-4 uppercase tracking-wider text-gray-800">Material</h4>
                                <div className="space-y-3">
                                    {materials.map((mat) => (
                                        <label key={mat} className="flex items-center gap-3 cursor-pointer group">
                                            <div className={`w-4 h-4 border flex items-center justify-center transition-colors ${selectedMaterials.includes(mat) ? 'bg-black border-black' : 'border-gray-300 group-hover:border-gray-400'}`}>
                                                {selectedMaterials.includes(mat) && <div className="w-2 h-2 bg-white" />}
                                            </div>
                                            <input 
                                                type="checkbox" 
                                                className="hidden"
                                            ...
                                            />
                                            <span className={`text-sm ${selectedMaterials.includes(mat) ? 'text-gray-900 font-medium' : 'text-gray-500 group-hover:text-gray-700'}`}>
                                                {mat}
                                            </span>
                                        </label>
                                    ))}
                            ...
                            <div>
                                 <h4 className="font-bold text-sm mb-4 uppercase tracking-wider text-gray-800">Price Range</h4>
